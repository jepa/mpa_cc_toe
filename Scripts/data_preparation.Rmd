...
mpa_sf <- dplyr::bind_rows(list(MPA1, MPA2, MPA3))
crs <- "+init=epsg:4326"

```

## Dissolve so that they are not overlapping 
This is necessary for the area calculation. Here is where you would select out just the Marine (2 or the marine and coastal, 2 and 1). You can also select out just the no take here. 

```{r}

mpa_sf$to_dissolve <- "A"

mpa_dissolve <- mpa_sf %>% 
  group_by(to_dissolve) %>% 
  summarize() %>% 
...
grid <- read_sf(here("./Data/spatial/worldsq_EA.shp")) %>% 
  ...
  st_transform(grid, crs = st_crs(crs))

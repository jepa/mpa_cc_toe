...
root_path <- "C:/Users/jepa88/Downloads"
}
...
#MPA metadata
grid_meta <- read_csv(paste0(root_path,"/data/mpa/grid_all_mpas.csv"))

# Get MPA prop grid
grid_mpa <- read.csv(paste0(root_path,"/data/mpa/mpa_index.txt"), header = F) %>% 
  select(fmort=V1)

head(grid_mpa)


#bind_cols(grid_meta,grid_mpa) %>% 
grid_final %>%  
  left_join(mpa_dbem_df) %>% 
 #filter(prop > 1) %>%
  ggplot() +
  geom_tile(
    aes(
